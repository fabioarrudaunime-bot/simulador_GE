...
function escalaInfo() {
    ...
    return { left: rect.left, top: rect.top, scaleX: rect.width / curso.largura, scaleY: rect.height / curso.altura };
}
...
function preloadProximos(s) {
    ...
    if (s.botoes) s.botoes.forEach(b => { if (b.destino) preloadSlide(b.destino); });
}

function abrirSlide(id, salvarHistorico = true) {
    if (!curso || !curso.slides[id]) { console.warn("Slide não encontrado:", id); return; }
    ...
    if (s.tempo && s.tempo > 0 && s.proximo) timer = setTimeout(() => abrirSlide(s.proximo), s.tempo * 1000);
}

function proximo() { const s = curso.slides[slideAtual]; if (s && s.proximo) abrirSlide(s.proximo); }
function anterior() { if (historico.length > 0) abrirSlide(historico.pop(), false); }
function reiniciar() { historico = []; abrirSlide(curso.slide_inicial, false); }
function voltarMenu() { window.location.href = (curso && curso.menu_url) ? curso.menu_url : "index.html"; }
function voltarPortal() { window.location.href = (curso && curso.portal_url) ? curso.portal_url : "index.html"; }
window.addEventListener("resize", () => { if (slideAtual) abrirSlide(slideAtual, false); });
document.addEventListener("keydown", e => { if (e.key === "ArrowRight") proximo(); if (e.key === "ArrowLeft") anterior(); if (e.key === "Home") voltarMenu(); if (e.key === "Escape") voltarPortal(); });
carregarCurso();
